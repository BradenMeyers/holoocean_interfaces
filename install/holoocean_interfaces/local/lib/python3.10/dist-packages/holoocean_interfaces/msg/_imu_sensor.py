    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'holoocean_interfaces.msg.IMUSensor')
            logger.debug(
...
class IMUSensor(metaclass=Metaclass_IMUSensor):
    ...
    _fields_and_field_types = {
        ...
        'acceleration_bias': 'float[3]',
        'angular_velocity_bias': 'float[3]',
    }
    ...
    def __init__(self, **kwargs):
        ...
        if 'acceleration_bias' not in kwargs:
            self.acceleration_bias = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.acceleration_bias = numpy.array(kwargs.get('acceleration_bias'), dtype=numpy.float32)
            assert self.acceleration_bias.shape == (3, )
        if 'angular_velocity_bias' not in kwargs:
            self.angular_velocity_bias = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.angular_velocity_bias = numpy.array(kwargs.get('angular_velocity_bias'), dtype=numpy.float32)
            assert self.angular_velocity_bias.shape == (3, )
    ...
    def __eq__(self, other):
        ...
        if all(self.angular_velocity != other.angular_velocity):
            return False
        if all(self.acceleration_bias != other.acceleration_bias):
            return False
        if all(self.angular_velocity_bias != other.angular_velocity_bias):
            return False
        return True
    ...
    @builtins.property
    def acceleration_bias(self):
        """Message field 'acceleration_bias'."""
        return self._acceleration_bias

    @acceleration_bias.setter
    def acceleration_bias(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.float32, \
                "The 'acceleration_bias' numpy.ndarray() must have the dtype of 'numpy.float32'"
            assert value.size == 3, \
                "The 'acceleration_bias' numpy.ndarray() must have a size of 3"
            self._acceleration_bias = value
            return
        if __debug__:
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -3.402823466e+38 or val > 3.402823466e+38) or math.isinf(val) for val in value)), \
                "The 'acceleration_bias' field must be a set or sequence with length 3 and each value of type 'float' and each float in [-340282346600000016151267322115014000640.000000, 340282346600000016151267322115014000640.000000]"
        self._acceleration_bias = numpy.array(value, dtype=numpy.float32)

    @builtins.property
    def angular_velocity_bias(self):
        """Message field 'angular_velocity_bias'."""
        return self._angular_velocity_bias

    @angular_velocity_bias.setter
    def angular_velocity_bias(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.float32, \
                "The 'angular_velocity_bias' numpy.ndarray() must have the dtype of 'numpy.float32'"
            assert value.size == 3, \
                "The 'angular_velocity_bias' numpy.ndarray() must have a size of 3"
            self._angular_velocity_bias = value
            return
        if __debug__:
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -3.402823466e+38 or val > 3.402823466e+38) or math.isinf(val) for val in value)), \
                "The 'angular_velocity_bias' field must be a set or sequence with length 3 and each value of type 'float' and each float in [-340282346600000016151267322115014000640.000000, 340282346600000016151267322115014000640.000000]"
        self._angular_velocity_bias = numpy.array(value, dtype=numpy.float32)
